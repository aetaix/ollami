...
export async function POST({ request }) {
	const { model, countryCode }: { model: Model, countryCode: string } = await request.json();
	console.log(model, countryCode);
	// GET https://newsapi.org/v2/top-headlines?country=us&apiKey=a97c79e9fc3a460cab028663d25a130b

	const getNews = await fetch(`https://newsapi.org/v2/top-headlines?sources=abc-news&apiKey=${NEWS_API_KEY}`, {
		method: 'GET',
		headers: {
