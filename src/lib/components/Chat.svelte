<script>
	...
	function handleSubmit() {
		...
		if ($input) {
			ram = true;
			append(
				{ role: 'user', content: $input, model: chatModel, rag},
				{
	...
	// Function to check and set the file states
	function checkAndSetFileStates(files) {
		if (files) {
			const filesData = JSON.parse(localStorage.getItem('files')) ?? [];
			const chatFiles = filesData.filter((f) => f.chat === $page.params.id);
			if (chatFiles.length > 0) {
				rag = { state: true, collection: collectionID };
			} else {
				rag.state = false;
			}
		}
